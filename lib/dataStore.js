...
/**
 * Provides the group list for a layer via EventEmitter. If the layer does not exist, no groups are
 * emitted.
 * 
 * Each group that is held by at least one member of the layer is emitted. The user list on each group
...
function load(dir, callback) {
	...
	function loadGroups(callback) {
		loadDir([ dir, 'groups' ].join('/'), function(filename, contents) {
			...
			if ( !group )
				return null;
			if ( !group.gidNumber ) {
				return console.log("Missing gidNumber for group " + filename);
			}
	...
	async.parallel([loadUsers, 
		...
		store = buildDataStore({
			dir: dir,
			users: compact(result[0]),
			groups: compact(result[1]),
