...
function logSearch(req, res, next) {
  log.info({requestId: req.logId, bindDN: req.connection.ldap.bindDN.toString()});
  log.info({requestId: req.logId, baseDN: req.dn.toString()});
  log.info({requestId: req.logId, filter: req.filter.toString()});
